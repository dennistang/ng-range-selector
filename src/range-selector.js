...
function rangeSelectorController($scope, $element, $attrs, $timeout) {
	...
	this.$onInit = () => {
		...
		$ctrl.updateRangeUI = () => {
			...
			let rangeInPixels = barWidth;
			let lowValueInPixels = Math.floor((lowValueInUnits / rangeInUnits) * rangeInPixels);
			let highValueInPixels = Math.floor((highValueInUnits / rangeInUnits) * rangeInPixels);
